#!/usr/bin/env bash
# scripts/download_proxy_model.sh
#
# Download DeepSeek-V2-Lite weights from HuggingFace.
#
# Why DeepSeek-V2-Lite?
# ──────────────────────────────────────────────────────────────────────────────
# DeepSeek-V2-Lite is a 15.7B-parameter MoE model that uses the *exact same*
# MLA (Multi-head Latent Attention) architecture as Kimi-K2.6:
#
#   input_layernorm → q_a_proj  → q_a_layernorm  → q_b_proj  → Q
#   input_layernorm → kv_a_proj → kv_a_layernorm → kv_b_proj → K, V
#
# It serves as a low-cost proxy for testing the RMSNorm+Linear fusion before
# applying it to the 2 TB Kimi-K2.6 model.
#
# Memory budget:
#   DeepSeek-V2-Lite BF16 ≈ 31 GB  (two copies ≈ 62 GB)
#   Single H100 80 GB has enough headroom for a dual-load numerical test.
#
...
# Approximate size in GiB (15.7B × 2 bytes BF16, plus config/tokenizer overhead)
APPROX_SIZE_GIB=35
...
if (( AVAILABLE_GIB < APPROX_SIZE_GIB )); then
    ...
    echo "  MODEL_DIR=/path/to/large/disk bash scripts/download_proxy_model.sh"
    exit 1
fi
echo "✔ Disk space OK"
...
# ── Download ──────────────────────────────────────────────────────────────────
echo ""
echo "Starting download — ~31 GB, should complete in a few minutes..."
echo ""
...
echo ""
echo " Next step — run the proxy fusion test:"
echo "   source .venv/bin/activate"
echo "   python tests/verify_fusion_proxy.py \\"
